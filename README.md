...
- **Soundscape** — the game looked far better than it sounded: every flight was one engine note regardless of speed, weather or stress. Three continuous noise layers now run off live state — slipstream that rises and brightens with airspeed, rain that drums on the airframe and muffles inside cloud, and a low rumble fed by turbulence, the wheels, and the buffet before a stall. All of them are filtered white noise from a 4-second loop, long enough that the period is not audible. Transients are scaled by the thing that caused them: a touchdown thump and tyre scrub driven by sink rate, so a greaser and an arrival sound as different as they score, and a gate whoosh that sweeps further the faster you go through it. Verified by measuring the master bus rather than by ear: quiet cruise peaks at 0.14, and worst case — full speed, heavy rain, rough air with thumps and whooshes together — peaks at 0.50, leaving 6 dB of headroom and never clipping.
...
`audioCtx` and `masterGain` are exposed so the mix can be measured rather than guessed at — attach an `AnalyserNode` to `masterGain` and read `getFloatTimeDomainData` to check levels and clipping. Note that gain changes use `setTargetAtTime`, so values converge over a few hundred milliseconds of *real* time; sampling immediately after setting state reads the old value.
